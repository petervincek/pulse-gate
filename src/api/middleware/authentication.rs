...
pub trait AuthService: Send + Sync + 'static {
    fn discover<'a>(&'a self) -> Pin<Box<dyn Future<Output = AnyhowResult<KeycloakMetadata>> + Send + 'a>>;
...
mod tests {
    ...
    use crate::{
        api::middleware::authentication::{AuthService, require_valid_token},
        app::state::AppState,
        service::keycloak::{KeycloakClaims, KeycloakMetadata, RealmAccess, VerifiedPrincipal},
    };
    ...
    impl AuthService for MockAuthService {
        fn discover<'a>(&'a self) -> std::pin::Pin<Box<dyn std::future::Future<Output = AnyhowResult<KeycloakMetadata>> + Send + 'a>> {
        ...
        fn validate_bearer_token<'a>(
            &'a self,
            bearer_token: &'a str,
        ) -> std::pin::Pin<Box<dyn std::future::Future<Output = AnyhowResult<VerifiedPrincipal>> + Send + 'a>> {
    ...
    fn test_app_state(auth: Arc<dyn AuthService>) -> AppState {
        ...
        let postgres_pool = PgPool::connect_lazy("postgres://postgres:postgres@127.0.0.1:5432/postgres")
    ...
    async fn middleware_rejects_missing_authorization_header() {
        ...
        let app = Router::new()
            ...
            .route_layer(middleware::from_fn_with_state(state.clone(), require_valid_token))
            .with_state(state);
    ...
    async fn middleware_accepts_valid_token_and_sets_principal_for_downstream_handler() {
        ...
        let app = Router::new()
            .route(
                "/secure",
                get(|Extension(principal): Extension<VerifiedPrincipal>| async move {
                    principal.subject
                }),
            )
            .route_layer(middleware::from_fn_with_state(state.clone(), require_valid_token))
